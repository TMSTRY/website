...
export default function HoverSwapText({
  ...
  return (
    <span className={`relative inline-block group/swap ${className}`}>
      ...
      <span
        aria-hidden="true"
        className={`font-grunge normal-case pointer-events-none absolute left-1/2 top-1/2 whitespace-nowrap opacity-0 group-hover/swap:opacity-100 transition-opacity duration-200 text-glow-pink ${altClassName}`}
        style={{
          transform: "translate(-50%, -50%) skewX(-12deg) rotate(-3deg)",
          textShadow: "0 0 14px rgba(224,64,251,0.45)",
        }}
      >
        {alt}
      </span>
